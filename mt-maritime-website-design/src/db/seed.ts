...
// --- Deterministic pseudo-random generator so seeding is repeatable ---
...
const prefixNames = [
  "Ocean", "Atlantic", "Pacific", "Northern", "Southern", "Arctic", "Tropic",
  "Coastal", "Deep Sea", "Blue", "Star", "Horizon", "Global", "Sovereign",
  "Majestic", "Royal", "Golden", "Silver", "Crimson", "Emerald", "Titan",
  "Neptune", "Poseidon", "Triton", "Mariner", "Voyager", "Pioneer", "Endeavour",
  "Guardian", "Sentinel", "Defender", "Vanguard", "Discovery", "Explorer",
  "Navigator", "Trailblazer", "Frontier", "Odyssey", "Legacy", "Liberty",
];

const suffixNames = [
  "Pride", "Spirit", "Endeavour", "Guardian", "Sentinel", "Star", "Wave",
  "Horizon", "Voyager", "Champion", "Falcon", "Eagle", "Phoenix", "Warrior",
  "Ranger", "Trader", "Runner", "Provider", "Supporter", "Venture", "Quest",
  "Titan", "Atlas", "Compass", "Current", "Tide", "Breeze", "Storm", "Dawn",
  "Summit", "Crest", "Point", "Bay", "Strait", "Reach", "Passage", "Anchor",
];
...
const flags = [
  "Panama", "Marshall Islands", "Singapore", "Liberia", "Malta", "United Kingdom",
  "Norway", "Vanuatu", "Cyprus", "Isle of Man", "India", "United Arab Emirates",
];

const homePorts = [
  "Singapore", "Aberdeen, UK", "Dubai, UAE", "Rotterdam, Netherlands",
  "Houston, USA", "Rio de Janeiro, Brazil", "Perth, Australia", "Lagos, Nigeria",
  "Mumbai, India", "Bergen, Norway", "Batam, Indonesia", "Port Klang, Malaysia",
];

const classifications = [
  "DNV", "ABS", "Lloyd's Register", "Bureau Veritas", "ClassNK", "RINA",
];
...
async function main() {
  ...
  const existing = await db.execute(sql`select count(*)::int as count from vessels`);
  const existingCount = Number((existing.rows[0] as { count: number }).count);
  if (existingCount > 0) {
    console.log(`Vessels table already has ${existingCount} rows. Skipping seed.`);
    await pool.end();
    return;
  }

  const usedNames = new Set<string>();
  const records: (typeof vessels.$inferInsert)[] = [];
  let globalId = 1;
  ...
  for (const def of vesselTypes) {
    for (let i = 0; i < def.count; i++) {
      ...
      records.push({
        ...
        lengthM: randFloat(def.lengthRange[0], def.lengthRange[1]).toString(),
        beamM: randFloat(def.beamRange[0], def.beamRange[1]).toString(),
        deadweightT: randInt(def.dwtRange[0], def.dwtRange[1]),
        bollardPullT: bollardPull,
        enginePowerBhp: randInt(def.bhpRange[0], def.bhpRange[1]),
        deckAreaSqm: def.deckAreaRange
          ? randInt(def.deckAreaRange[0], def.deckAreaRange[1])
          : null,
        crewCapacity: randInt(def.crewRange[0], def.crewRange[1]),
  ...
  console.log(`Seeded ${records.length} vessels.`);
  await pool.end();
}
